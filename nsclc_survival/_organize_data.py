...
def organize_dicom_data(raw_path, organized_path):
    ...
    if not raw_path.exists():
        logger.warning(f"Warning: raw folder not found: probably all files have already been moved and the raw folder removed.")
        return

    all_folders = [f for f in raw_path.iterdir() if f.is_dir()]

    logger.info(f"Analysing {len(all_folders)} folders in progress...")

    if len(all_folders) == 0:
        logger.info("All folders have already been moved!")
    else:
        all_successful = True
        ...
        for folder_path in all_folders:
            ...
            if files:
                try:
                    ...
                    logger.info(f"Moved: {patient_id} - {modality}")
        ...
        if all_successful:
            try:
                ...
                logger.error(f"Could not remove folder {raw_path}: {e}")
        else:
